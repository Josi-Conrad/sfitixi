...
use Tixi\ApiBundle\Interfaces\Management\VehicleDepotListDTO;
...
/**
 * Class VehicleDepotDataGridController
 * @package Tixi\ApiBundle\Shared\DataGrid\GridControllers\Management
 */
class VehicleDepotDataGridController extends DataGridAbstractController {
    /**
     * @return mixed|string
     */
    ...
    public function createCustomControlTile()
    {
        ...
        $selectionButton->add(new TextLinkSelectionTile('edit', $this->generateUrl('tixiapi_management_vehicledepot_edit',array('vehicleDepotId'=>DataGridHandler::$dataGirdReplaceIdentifier)),'button.edit',true));
        $selectionButton->add(new SelectionButtonDividerTile());
        $selectionButton->add(new TextLinkSelectionDeleteTile('delete', $this->generateUrl('tixiapi_management_vehicledepot_delete',array('vehicleDepotId'=>DataGridHandler::$dataGirdReplaceIdentifier)),'button.delete',true));
    ...
    {
        return $this->generateUrl('tixiapi_management_vehicledepot_edit',array('vehicleDepotId'=>DataGridHandler::$dataGirdReplaceIdentifier));
    }

    /**
     * @return mixed|VehicleDepotListDTO
     */
    public function getReferenceDTO()
    {
        if(!$this->isInEmbeddedState()) {
            return new VehicleDepotListDTO();
        }
    }
    ...
    public function constructDtosFromFgeaFilter(GenericEntityFilter $filter)
    {
        ...
        $assembler = $this->container->get('tixi_api.assemblervehicledepot');
        $vehicleDepots = $this->getEntitiesByFgeaFilter($filter);
        $dtos = array();
        if(!$this->isInEmbeddedState()) {
            $dtos = $assembler->vehicledepotsToVehicleDepotListDTOs($vehicleDepots);
        }
        return $dtos;
    }
