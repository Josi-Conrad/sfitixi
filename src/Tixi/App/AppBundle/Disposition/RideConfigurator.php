...
class RideConfigurator {
    ...
    public function __construct($drivingMissions, $drivingPools, $vehicles, $type = self::ONLY_TIME_WINDOWS) {
        $this->rideNodes = $this->createPassengerRideNodesFromDrivingMissions($drivingMissions);
...
//                                $currentNode = $list->pop();
    ...
    protected function buildConfigurations(){
        $this->rideConfigurations = array();

        $workRideNodes = $this->rideNodes;
        $this->sortNodesByStartMinute($workRideNodes);

        //build some (random) configs
        for($i=0; $i<10; $i++){
            $rideConfiguration = new RideConfiguration();
            foreach($this->drivingPools as $drivingPool){
                $rideNode = new RideNodeList();


                $rideConfiguration->addRideNodeList($rideNode);
            }
            $this->rideConfigurations[] = $rideConfiguration;
        }
    }
